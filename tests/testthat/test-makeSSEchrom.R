...
test_that("makeSSEchrom state.names labels binary states", {
  skip_if_not_installed("diversitree")
  set.seed(42)
  result <- makeSSEchrom(
    h = 10, lambda1 = 0.3, lambda2 = 0.3,
  ...
  expect_type(result, "list")
  expect_named(result, c("tree", "binary.char", "haploid.num"))
  expect_true(all(result$binary.char %in% c("matched", "mismatched")))
})

test_that("makeSSEchrom state.names validates input", {
  skip_if_not_installed("diversitree")
  expect_error(
    makeSSEchrom(
      h = 10, lambda1 = 0.3, lambda2 = 0.3,
